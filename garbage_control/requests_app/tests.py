...
class RequestWorkflowApiTests(APITestCase):
    ...
    def test_citizen_can_create_request(self, mock_detect_city, mock_detect_before):
        ...
        self.assertEqual(request_obj.locality_id, self.locality.id)
        self.assertEqual(request_obj.municipality_id, self.municipality.id)
        self.assertEqual(request_obj.federal_subject_id, self.subject.id)
