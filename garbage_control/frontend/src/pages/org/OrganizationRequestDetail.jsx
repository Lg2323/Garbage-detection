...
export default function OrganizationRequestDetail() {
  ...
  useEffect(() => {
    load().catch((error) => {
      setMsg({
        type: "danger",
        text: "Ошибка загрузки: " + (error.response?.data ? JSON.stringify(error.response.data) : error.message),
      });
    });
  }, [id]);
  ...
  const brigades = useMemo(() => {
    ...
    const orgId = options.current_user?.organization || requestItem?.responsible_organization;
    if (orgId) {
      filtered = filtered.filter((brigade) => brigade.organization_id === Number(orgId));
    }
    if (form.responsible_department) {
      filtered = filtered.filter(
        (brigade) => brigade.department_id === Number(form.responsible_department)
      );
    }
    return filtered;
  }, [options, requestItem?.responsible_organization, form.responsible_department]);
  ...
  const workers = useMemo(() => {
    ...
    const orgId = options.current_user?.organization || requestItem?.responsible_organization;
    if (orgId) {
      filtered = filtered.filter((worker) => worker.organization_id === Number(orgId));
    }
    if (form.responsible_department) {
      filtered = filtered.filter(
        (worker) => !worker.department_id || worker.department_id === Number(form.responsible_department)
      );
    }
    return filtered;
  }, [options, requestItem?.responsible_organization, form.responsible_department]);
  ...
  const submit = async () => {
    ...
    try {
      ...
      await load();
      setMsg({ type: "success", text: "Маршрут исполнения обновлён." });
    } catch (error) {
      setMsg({
        type: "danger",
  ...
  return (
    <div className="d-grid gap-3 gc-anim gc-anim--up">
      ...
      <div className="row g-3">
        ...
        <div className="col-lg-6">
          <div className="card p-3 h-100">
            <div className="fw-semibold mb-2">Назначение внутри организации</div>
            <div className="row g-3">
              ...
              <div className="col-12">
                ...
                <textarea
                  ...
                  placeholder="Что именно назначено и в какой срок"
                />
              </div>
            </div>
              ...
              {requestItem.assignments?.length ? (
                requestItem.assignments.map((assignment) => (
                  <div key={assignment.id} className="gc-timeline__item">
                    ...
                    <div>
                      {assignment.assigned_department_name || assignment.assigned_brigade_name || assignment.assigned_worker_username || "-"}
                    </div>
